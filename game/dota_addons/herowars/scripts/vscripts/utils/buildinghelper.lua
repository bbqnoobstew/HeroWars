...
function BuildingHelper:BlockRectangularArea(leftBorderX, rightBorderX, topBorderY, bottomBorderY)
	...
	for x=leftBorderX+32, rightBorderX-32, 64 do
		...
		print("Total Squares blocked: " .. blockedCount)
end
...
function BuildingHelper:AddBuilding(building)
	...
	--adding the below three lines NJG 2015/01/01
	if not building.BHOwner:IsRealHero() then 
		local ownerPlayer = building.BHOwner:GetOwner()
		local ownerPlayerID = ownerPlayer:GetPlayerID()
		building:SetControllableByPlayer(ownerPlayerID, true)
	else
